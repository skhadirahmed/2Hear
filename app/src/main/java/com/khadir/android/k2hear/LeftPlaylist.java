...
public class LeftPlaylist extends AppCompatActivity {
    ...
    public void addSongs(View view) {
        ...
        startActivityForResult(intent, MusicMainActivity.LEFT_REQUEST_CODE);
    }

    @Override
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        super.onActivityResult(requestCode, resultCode, data);
        if (requestCode == MusicMainActivity.LEFT_REQUEST_CODE) {
...
//                Bitmap bitmap = BitmapFactory.decodeFile(path);
                ...
                MusicMainActivity.LalbumArt.setBackground(drawable);
                Log.v("LeftPlaylist", "data from getData() is " + data);
                if (MusicMainActivity.LeftMediaPlayer.isPlaying()) {
                    //Clear the media player object for the next song
                    MusicMainActivity.LeftMediaPlayer.stop();
                    MusicMainActivity.LeftMediaPlayer.reset();
                }
                try {
                    Toast.makeText(LeftPlaylist.this, "preparing the medaiplayer !", Toast.LENGTH_SHORT).show();
                    MusicMainActivity.LeftMediaPlayer.setDataSource(data);
                    MusicMainActivity.LeftMediaPlayer.prepare();
                } catch (IOException e) {
                    e.printStackTrace();
                }
                MusicMainActivity.LeftMediaPlayer.start();
//                MusicMainActivity.LeftMediaPlayer.setOnCompletionListener(new MediaPlayer.OnCompletionListener() {
//                    @Override
