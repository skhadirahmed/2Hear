...
import com.khadir.android.k2hear.AllMusic;
...
public class RightPlaylist extends AppCompatActivity {
    ...
    public void addSongs(View view) {
        ...
        startActivityForResult(intent, MusicMainActivity.RIGHT_REQUEST_CODE);
    }

    @Override
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        super.onActivityResult(requestCode, resultCode, data);
        if (requestCode == MusicMainActivity.RIGHT_REQUEST_CODE) {
...
//                Bitmap bitmap = BitmapFactory.decodeFile(path);
                ...
                MusicMainActivity.RalbumArt.setBackground(drawable);
//                Log.v("LeftPlaylist", "data from getData() is " + data);
                if (MusicMainActivity.RightMediaPlayer.isPlaying()) {
                    //Clear the media player object for the next song
                    MusicMainActivity.RightMediaPlayer.stop();
                    MusicMainActivity.RightMediaPlayer.reset();
                }
                try {
                    Toast.makeText(RightPlaylist.this, "preparing the medaiplayer !", Toast.LENGTH_SHORT).show();
                    MusicMainActivity.RightMediaPlayer.setDataSource(data);
                    MusicMainActivity.RightMediaPlayer.prepare();
                } catch (IOException e) {
                    e.printStackTrace();
                }
                MusicMainActivity.RightMediaPlayer.start();
//                MusicMainActivity.RightMediaPlayer.seekTo(200000);
//                MusicMainActivity.RightMediaPlayer.setOnCompletionListener(new MediaPlayer.OnCompletionListener() {
//                    @Override
